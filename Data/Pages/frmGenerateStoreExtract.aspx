<%@ Page Title="" Language="C#" MasterPageFile="~/Data/MasterPage/Site.master" AutoEventWireup="true" CodeFile="frmGenerateStoreExtract.aspx.cs" Inherits="frmGenerateStoreExtract" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function fnShowReport(flg) {
            $("#divloader").show();
            var RoleId = $("#ConatntMatter_hdnNodeID").val();
            var LoginId = $("#ConatntMatter_hdnLoginId").val();
            PageMethods.GetReport(RoleId, LoginId, flg, GetReport_pass, fnFailed, flg);
        }
        function GetReport_pass(result, flg) {
            $("#divloader").hide();

            if (result.split("|")[0] == "0") {

                if (flg == 0) {
                    $("#ConatntMatter_ddlMonthYear").html(result.split("|")[1]);
                }

                $("#divReport").html(result.split("|")[2]);
        ...
        function fnIndividualSave() {
             var Month = $("#ConatntMatter_ddlMonthYear").val()
            ...
            if (Month == "0") {
                ...
                var LoginId = $("#ConatntMatter_hdnLoginId").val();
                $("#lblMsg").html("Extract is being generated.... , kindly check after 15mins");
                PageMethods.fnSave(Month, Year, LoginId, fnSave_pass, fnFailed);
            }
        }
        function fnSave_pass(res, Img) {
            if (res.split("|^|")[0] == "0") {
                $("#lblMsg").html("Extract Generated successfully");
                fnShowReport(1);
            }
            else {
                $("#lblMsg").html(res.split("|^|")[1]);
...
<asp:Content ID="Content3" ContentPlaceHolderID="ConatntMatter" runat="Server">
    <div class="card default_bg no_radius m_row">
        <div class="card-header green_bg pr-0">
            <div class="card-title-heading white_txt" id="divRptTitle" style="display: inline-block; width: 100%; text-align: center;">Store Extract</div>
           
        </div>
        <div class="ml-4 mt-3">
            <span style="font-size: 1rem; font-weight: 700;">Month-Year : </span>
            <asp:DropDownList ID="ddlMonthYear" runat="server" CssClass="form-control form-control-sm d-inline-block bg-transparent ml-2"  Style="width: 200px;"></asp:DropDownList>
         <input type="button" class="btns btn-submit" onclick="fnIndividualSave()" value="Save" />
            <label id="lblMsg" style="margin:5px 10px"></label>
        </div>
