...
public class FileUpload : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    private string UploadFile(HttpPostedFile File_Up, string FileType, string LoginID, string UserID, string MonthYr, HttpContext context)
    {
        ...
        try
        {
            ...
            if (!File.Exists(HttpContext.Current.Server.MapPath("~/Files/" + FolderName + "/Error")))
                Directory.CreateDirectory(HttpContext.Current.Server.MapPath("~/Files/" + FolderName + "/Error"));

            //Save the uploaded file at new location.
            string filename = FolderName + "_" + LoginID + "_" + UserID + "_" + MonthYr.Replace('|', '_') + "_" + DateTime.Now.ToString("dd-MM-yyyy-hh-mm-ss") + Path.GetExtension(File_Up.FileName);
            string filePath = HttpContext.Current.Server.MapPath("~/Files/" + FolderName + "/Temp/") + filename;
            ...
            using (FileStream oStream = File.Open(filePath, FileMode.Open, FileAccess.Read))
            {
                ...
                DsExcelData = ToAllStringFields(iExcelDataReader.AsDataSet(conf));
            }

            if (FileType == "1" && DsExcelData.Tables[0].Rows.Count > 10000)
                return "1^You can only allow to upload Data upto 10000 records ! ";
            ...
            switch (FileType)
            {
                case "1":
                    ...
                    break;
                case "2":
                    Scmd.Parameters.AddWithValue("@Descr", "HR - Employee Master");
                    break;
                case "3":
                    Scmd.Parameters.AddWithValue("@Descr", "HR - Payroll");
                    break;
            }
            ...
            {
                if (File.Exists(HttpContext.Current.Server.MapPath("~/Files/" + FolderName + "/Temp/") + filename))
                    File.Move(HttpContext.Current.Server.MapPath("~/Files/" + FolderName + "/Temp/") + filename, HttpContext.Current.Server.MapPath("~/Files/" + FolderName + "/") + filename);

                if (ds.Tables.Count > 2)
                {
                    if (ds.Tables[2].Rows.Count > 0)
                        return "0^<div style='margin: 10px 0; font-weight: 600;'>" + FolderName + " details Updated successfully at " + DateTime.Now.ToString("dd-MMM-yyyy HH:mm:ss") + ", except for the following Branch(s) :</div>" + DatatabletoStr(ds.Tables[2], "OtherUserBranch") + "^" + ds.Tables[1].Rows[0]["FileID"].ToString();
                    else
                        return "0^" + FolderName + " details Updated successfully at " + DateTime.Now.ToString("dd-MMM-yyyy HH:mm:ss") + ".^" + ds.Tables[1].Rows[0]["FileID"].ToString();
                }
                else
                    return "0^" + FolderName + " details Updated successfully at " + DateTime.Now.ToString("dd-MMM-yyyy HH:mm:ss") + ".^" + ds.Tables[1].Rows[0]["FileID"].ToString();
            }
            ...
            {
                if (File.Exists(HttpContext.Current.Server.MapPath("~/Files/" + FolderName + "/Temp/") + filename))
                    File.Move(HttpContext.Current.Server.MapPath("~/Files/" + FolderName + "/Temp/") + filename, HttpContext.Current.Server.MapPath("~/Files/" + FolderName + "/Error/") + filename);
    ...
    public static DataSet ToAllStringFields(DataSet ds)
    {
